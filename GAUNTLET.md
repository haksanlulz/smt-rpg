...
| 2026-07-28 | **The Pay Out Rewards button was cramped and ran off the sidebar.** Reported from play. `.smt-grant-rewards` had **zero rules anywhere in the stylesheet** — the button was appended straight into Foundry's `.combat-controls`, a bar sized for icon-only buttons, and inherited whatever that gave it. This is the "not rendering right" report that had been open for weeks with no mechanism attached. It now gets its own full-width row inserted *after* the controls bar, and an actual rule set. **A second, self-inflicted instance shipped the same day:** `.effect-actions` never declared `flex-wrap`, and the Aid button added that morning made it a three-button nowrap row inside a fixed-width sheet column. Its sibling `.effects-list` had wrapped all along. | **New scan C13** — every `smt-` class the system applies has a style rule or is a `querySelector` hook. Mutation-proved (probe now 18/18) and separately proved red against the real defect by removing the rule. ⚑ **It found two more dead classes on its first run**, `smt-chat` and `smt-levelup-effects`, both applied and never styled; the second is a bare `<ul>` taking the browser's ~40px indent inside a chat card, i.e. a third overflow nobody had reported. ⚑ **The first attempt to prove C13 red came back green and was wrong** — the backup copy of the stylesheet had been left inside the scanned tree, so the rules were still found. The instrument was fine; the way it was aimed was not. |
