...
|---|---|---|
| Drain Attack | "recover HP equal to 25% of the damage dealt" | **implementable now** — damage-pipeline hook, no dependency |
| Attack All | "basic strikes always target all enemies" | **implementable now** — targeting, and the Counter exclusion is already a flag |
| Item Pro | "when using items, add 1d10 to the power roll" | **implementable now** — consumable power path |
| Luck Smiles | "completely nullify the effects of an attack on you, 1/scenario" | **implementable now** — reaction; the scenario budget already exists |
