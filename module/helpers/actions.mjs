...
// document.
//
// p.63 states the base rule twice and leaves no room: "Each combatant gets one turn per
// round", then "During their turn in the initiative order, combatants may take one
// action. An 'action' is one of the following: making a basic strike, using a skill,
// talking, aiding, concentrating, defending, or using an item." The boss trait is the
// only printed change to the number — "Bosses take two actions on their turn" (p.278).
//
// p.96 PRESS SKILLS: "Beast Eye and Dragon Eye are skills that can increase how many
// actions one can take per turn. They cost one action to apply. Beast Eye spends one
// action to grant two actions, effectively granting one additional action. Dragon Eye
// takes one action to grant four actions, effectively granting three additional
// actions." So the grant is GROSS, not net — spending is charged separately, and the
// book's "effectively" arithmetic falls out rather than being hardcoded.
//
